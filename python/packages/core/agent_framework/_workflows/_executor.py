...
def _validate_handler_signature(
    ...
    *,
    ...
    # Resolve postponed/forward-referenced annotations to concrete types for downstream validation.
    # This enables `from __future__ import annotations` on executors.
    ...
    try:
        ...
        # If annotations are stringized and cannot be resolved, raise a clearer error.
        if isinstance(message_param.annotation, str) or isinstance(ctx_param.annotation, str):
            raise ValueError(
                ...
                "Ensure referenced types are imported/defined in the handler module, or avoid unresolved forward refs."
            ) from exc
        type_hints = {}
