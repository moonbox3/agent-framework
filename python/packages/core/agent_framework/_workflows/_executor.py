...
def _validate_handler_signature(
    ...
    *,
    ...
    # Best-effort resolution of postponed annotations (PEP 563 / Python 3.12 common case)
    try:
        type_hints = typing.get_type_hints(func, globalns=getattr(func, "__globals__", None), localns=None)
    ...
    if skip_message_annotation and ctx_annotation == inspect.Parameter.empty:
        ...
        if isinstance(ctx_annotation, str):
            # Narrow fallback: resolve stringified ctx annotation when get_type_hints
            # couldn't fully evaluate (e.g. missing names at decoration time).
