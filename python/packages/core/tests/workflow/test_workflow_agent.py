...
class _ReasoningEmittingExecutor(Executor):
    ...
    def __init__(
        self,
        id: str,
        intermediate_data: AgentResponse | Message | list[Message],
        terminal_output: AgentResponse | Message | list[Message],
    ):
        super().__init__(id=id)
        self._intermediate_data = intermediate_data
    ...
    async def handle(
        ...
        await ctx.add_event(WorkflowEvent.emit(self.id, self._intermediate_data))
        await ctx.yield_output(self._terminal_output)


class TestWorkflowAgentReasoningHelpers:
    """Tests for WorkflowAgent._rewrite_text_to_reasoning and _msg_as_reasoning helpers."""

    def test_rewrite_text_to_reasoning_converts_text(self) -> None:
        """Text content blocks are converted to text_reasoning, preserving id and text."""
        text = Content.from_text(text="hello world", additional_properties={"src": "agent1"})
        result = WorkflowAgent._rewrite_text_to_reasoning([text])
        assert len(result) == 1
        assert result[0].type == "text_reasoning"
        assert result[0].text == "hello world"  # type: ignore[attr-defined]
        assert result[0].additional_properties.get("src") == "agent1"

    def test_rewrite_text_to_reasoning_passes_through_function_call(self) -> None:
        """Non-text content (function calls, results) passes through unchanged."""
        fc = Content.from_function_call(call_id="c1", name="my_tool", arguments={"x": 1})
        result = WorkflowAgent._rewrite_text_to_reasoning([fc])
        assert len(result) == 1
        assert result[0] is fc  # same instance — passed through

    def test_rewrite_text_to_reasoning_no_double_wrap(self) -> None:
        """Already-reasoning content stays as text_reasoning (not wrapped again)."""
        already = Content.from_text_reasoning(text="thinking")
        result = WorkflowAgent._rewrite_text_to_reasoning([already])
        assert len(result) == 1
        assert result[0] is already  # same instance — only type=='text' is rewritten

    def test_rewrite_text_to_reasoning_handles_mixed_content(self) -> None:
        """Mixed content: only text blocks are rewritten; others pass through."""
        text = Content.from_text(text="answer")
        fc = Content.from_function_call(call_id="c1", name="my_tool", arguments={})
        already = Content.from_text_reasoning(text="prior thinking")
        result = WorkflowAgent._rewrite_text_to_reasoning([text, fc, already])
        assert [c.type for c in result] == ["text_reasoning", "function_call", "text_reasoning"]
        assert result[1] is fc
        assert result[2] is already

    def test_msg_as_reasoning_preserves_role_and_metadata(self) -> None:
        """_msg_as_reasoning copies the message with rewritten contents but preserves all other fields."""
        original = Message(
            "assistant",
            [Content.from_text(text="hi"), Content.from_function_call(call_id="c1", name="t", arguments={})],
            author_name="agent1",
            message_id="msg-123",
            additional_properties={"meta": "value"},
        )
        new_msg = WorkflowAgent._msg_as_reasoning(original)
        assert new_msg is not original
        assert new_msg.role == "assistant"
        assert new_msg.author_name == "agent1"
        assert new_msg.message_id == "msg-123"
        assert new_msg.additional_properties.get("meta") == "value"
        assert [c.type for c in new_msg.contents] == ["text_reasoning", "function_call"]
        # Original message is unmodified
        assert [c.type for c in original.contents] == ["text", "function_call"]


class TestWorkflowAgentDataEventReasoningConversion:
    """End-to-end tests for as_agent() rewriting data event content as reasoning."""

    async def test_data_event_text_becomes_reasoning_non_streaming(self) -> None:
        """A data event carrying AgentResponse with text content surfaces as text_reasoning in as_agent()."""
        intermediate = AgentResponse(messages=[Message("assistant", [Content.from_text(text="thinking step")])])
        terminal = AgentResponse(messages=[Message("assistant", [Content.from_text(text="final answer")])])
        exec_ = _ReasoningEmittingExecutor(id="exec_a", intermediate_data=intermediate, terminal_output=terminal)
        agent = WorkflowBuilder(start_executor=exec_).build().as_agent()

        response = await agent.run("go")

        assert isinstance(response, AgentResponse)
        all_types = [c.type for m in response.messages for c in m.contents]
        # Intermediate content rewritten to text_reasoning; terminal stays text.
        assert "text_reasoning" in all_types
        assert "text" in all_types
        reasoning_text = " ".join(
            c.text or "" for m in response.messages for c in m.contents if c.type == "text_reasoning"
        )
        answer_text = " ".join(c.text or "" for m in response.messages for c in m.contents if c.type == "text")
        assert reasoning_text == "thinking step"
        assert answer_text == "final answer"

    async def test_output_event_text_passes_through_non_streaming(self) -> None:
        """An output event with text content passes through unchanged (not rewritten)."""

        class _OutputOnly(Executor):
            @handler
            async def handle(self, message: list[Message], ctx: WorkflowContext[Any, Any]) -> None:
                response = AgentResponse(messages=[Message("assistant", [Content.from_text(text="answer")])])
                await ctx.yield_output(response)

        agent = WorkflowBuilder(start_executor=_OutputOnly(id="solo")).build().as_agent()
        response = await agent.run("go")
        all_types = [c.type for m in response.messages for c in m.contents]
        assert all_types == ["text"], "Output events must not be rewritten as reasoning"

    async def test_data_event_with_mixed_content_only_text_rewritten(self) -> None:
        """In a data event, only text content is rewritten; function_call/result pass through."""
        intermediate = AgentResponse(
            messages=[
                Message(
                    "assistant",
                    [
                        Content.from_text(text="reasoning"),
                        Content.from_function_call(call_id="c1", name="search", arguments={"q": "x"}),
                    ],
                )
            ]
        )
        terminal = AgentResponse(messages=[Message("assistant", [Content.from_text(text="done")])])
        exec_ = _ReasoningEmittingExecutor(id="mix", intermediate_data=intermediate, terminal_output=terminal)
        agent = WorkflowBuilder(start_executor=exec_).build().as_agent()
        response = await agent.run("go")

        # Find the message that has the function_call (the intermediate one)
        intermediate_msg = next(m for m in response.messages if any(c.type == "function_call" for c in m.contents))
        types = [c.type for c in intermediate_msg.contents]
        assert types == ["text_reasoning", "function_call"]

    async def test_data_event_already_reasoning_not_double_wrapped(self) -> None:
        """A data event whose content is already text_reasoning surfaces unchanged (no double wrap)."""
        intermediate = AgentResponse(
            messages=[Message("assistant", [Content.from_text_reasoning(text="already thinking")])]
        )
        terminal = AgentResponse(messages=[Message("assistant", [Content.from_text(text="done")])])
        exec_ = _ReasoningEmittingExecutor(id="reasoning_in", intermediate_data=intermediate, terminal_output=terminal)
        agent = WorkflowBuilder(start_executor=exec_).build().as_agent()
        response = await agent.run("go")

        reasoning_blocks = [c for m in response.messages for c in m.contents if c.type == "text_reasoning"]
        assert len(reasoning_blocks) == 1
        assert reasoning_blocks[0].text == "already thinking"  # type: ignore[attr-defined]

    async def test_data_event_text_becomes_reasoning_streaming(self) -> None:
        """In streaming mode, AgentResponseUpdate from data events carries text_reasoning content."""
        intermediate = AgentResponse(messages=[Message("assistant", [Content.from_text(text="midway")])])
        terminal = AgentResponse(messages=[Message("assistant", [Content.from_text(text="end")])])
        exec_ = _ReasoningEmittingExecutor(id="stream_x", intermediate_data=intermediate, terminal_output=terminal)
        agent = WorkflowBuilder(start_executor=exec_).build().as_agent()

        updates: list[AgentResponseUpdate] = []
        async for update in agent.run("go", stream=True):
            updates.append(update)

        all_types = [c.type for u in updates for c in u.contents]
        assert "text_reasoning" in all_types
        assert "text" in all_types
        # Reasoning chunk's text matches the intermediate
        reasoning_chunks = [c for u in updates for c in u.contents if c.type == "text_reasoning"]
        assert any((c.text or "") == "midway" for c in reasoning_chunks)
        # Terminal text chunk matches
        text_chunks = [c for u in updates for c in u.contents if c.type == "text"]
        assert any((c.text or "") == "end" for c in text_chunks)
    ...
    async def test_data_event_streaming_does_not_mutate_source_update(self) -> None:
        ...
        agent = WorkflowBuilder(start_executor=_SharedUpdateExecutor(id="src")).build().as_agent()
        async for _ in agent.run("go", stream=True):
            pass

        # Source update content must be unchanged (still `text`, never rewritten to `text_reasoning`).
        assert [c.type for c in original_update.contents] == ["text"]
        assert original_update.author_name == "agent_a"  # not stamped with executor id
