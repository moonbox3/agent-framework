...
history sent to each agent. This is useful for:
...
def create_agents(chat_client: OpenAIChatClient) -> tuple[ChatAgent, ChatAgent, ChatAgent]:
    ...
    triage = chat_client.create_agent(
        instructions=(
            ...
            "technical_agent for technical problems or billing_agent for billing issues. "
            "Include 'HANDOFF_TO: <agent_name>' when routing. Be concise."
        ),
        name="triage_agent",
    )
...
def _print_handoff_request(request: HandoffUserInputRequest) -> None:
    ...
    print(f"Context available to agents: Last {len(request.conversation)} messages")
    for i, message in enumerate(request.conversation, 1):
        speaker = message.author_name or message.role.value
        # Truncate long messages for display
