...
async def to_analysis_result(response: AgentExecutorResponse, ctx: WorkflowContext[AnalysisResult]) -> None:
    parsed: AnalysisResultAgent
    if response.agent_run_response.value is not None:
        parsed = cast(AnalysisResultAgent, response.agent_run_response.value)
    else:
...
async def finalize_and_send(response: AgentExecutorResponse, ctx: WorkflowContext[Never, str]) -> None:
    parsed: EmailResponse
    if response.agent_run_response.value is not None:
        parsed = cast(EmailResponse, response.agent_run_response.value)
    else:
